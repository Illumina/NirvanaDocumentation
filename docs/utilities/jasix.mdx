...
The Jasix index is aimed at providing TABIX like indexing capabilities for the Illumina Annotator JSON output.

## Creating the Jasix index

The Jasix index (that comes in a .jsi) file is generated on-the-fly with Illumina Annotator output. It can also be generated independently by running the Jasix command line utility on the JSON output file. Please note that the Jasix utility can only consume JSON files that follow the Illumina Annotator JSON output format. The following code blocks demonstrate the help menu and index generating functionalities of Jasix.
...
Indexes a Illumina Annotator annotated JSON file
...
The Illumina Annotator JSON file has three sections: header, positions and genes. Header can be printed using the -H option. If you are interested in only the positions or genes section, you can use the -s or --section option.
