...
The ClinVar ``.nsa`` for Nirvana can be build using the ``SAUtils`` command's ``clinvar`` subcommand.
### Source data files
Two input ``.xml`` files and a ``.version`` file is required in order to build the ``.nsa`` file. Here is a set of files.
