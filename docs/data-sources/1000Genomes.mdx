...
All frequency related information comes in comma separated allele specific fields and are parsed into allele specific annotations. The ancestral allele, if it exists, is the first value in the pipe separated AA fields (the Indel specific REF, ALT, IndelType fields are ignored). However, the actual allele counts and allele numbers (e.g. AMR_AC, AMR_AN) are not expressed in the INFO field. Instead the genotypes need to be parsed to directly compute that information. Our team converted the original data to VCF entries like the following:
...
```
In order to get 6 digit precision, allele frequencies are computed from allele counts and allele numbers
...
| CNV    | TRUE                              | copy_number_loss (observed_gains = 0 and observed_losses > 0) |
