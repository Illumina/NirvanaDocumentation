...
## Small Variants
### VCF File Parsing
We parse the 1000 Genomes vcf file and extract the following fields:
* AC
* AN
* EAS_AN
* AMR_AN
...
**NOTE**: The actual allele counts and allele numbers are not expressed in the INFO field. Instead the genotypes need to be parsed to directly compute that information. Our team converted the original data to VCF entries like the following:
...
```
Allele frequencies are computed from allele counts and allele numbers in order to get 6 digit precision.
#### Conflict Resolution
